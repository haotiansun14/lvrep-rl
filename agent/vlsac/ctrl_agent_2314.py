...
class CTRL_SACAgent(SACAgent):
	...
	def __init__(
		...
		self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), 
										  weight_decay=0, lr=lr/5, betas=[0.9, 0.999]) # wd 1e-3
		self.log_alpha_optimizer = torch.optim.Adam([self.log_alpha], lr=lr/5, betas=[0.9, 0.999])
		# lr decay
    ...
    #   model_losses.append(tf.reduce_mean(ce_loss))
		...
		model_loss = (z_phi[:, None, :] * z_mu_next[None, :, :]).sum(-1) #/ 0.2
